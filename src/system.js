...
const Link = ({ children, ...props }) => {
  return (
    <a
      className={c('text-orange-800 hover:text-orange-600 transition-all')}
      {...props}
    >
      {children}
    </a>
...
const TitleContainer = ({ alignment = 'left', children }) => (
  <div className={c('my-8', [{ 'text-align': `text-${alignment}` }])}>
    {children}
  </div>
)
...
const Title = ({ alignment, children, size, ...props }) => {
  ...
  switch (size) {
    case 'h1':
      tag = (
        <h1 {...props} className={c('text-4xl', 'leading-tight', baseClasses)}>
          {children}
        </h1>
      )
      break
    case 'h2':
      tag = (
        <h2 {...props} className={c('text-3xl', baseClasses)}>
          {children}
        </h2>
      )
      break
